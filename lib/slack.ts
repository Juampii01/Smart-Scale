...
// ─── Notification: cliente aprobó un nivel de POSI ────────────────────────────
// A diferencia de las demás notificaciones de este archivo, esta se postea en
// el canal PROPIO del cliente (no uno interno de equipo) — la ve el cliente,
// tono de festejo. Requiere que omni_slack_channels.client_id esté poblado
// para ese cliente (correr POST /api/admin/omni/slack/sync si no matchea).
//
// No hay Slack user ID guardado de los clientes hoy, así que no se puede
// taguear con una mención real (<@U...>) — se usa el nombre en texto.

const LEVEL_UP_TEMPLATES: ((name: string, level: string) => string)[] = [
  (name, level) =>
    `🎉 *¡Felicitaciones, ${name}!* 🏆\n¡Qué gran trabajo completando *${level}*! Seguí así, vamos por más. 💪💪💪`,
  (name, level) =>
    `🚀 *Welcome to the next level, ${name}!* 🏆\nAcabás de aprobar *${level}* — tu progreso es una locura, seguí empujando. 🔥🔥🔥`,
  (name, level) =>
    `👏 *${name} rompió otro nivel!* 🏆\n*${level}* → ¡aprobado! Cada paso te acerca más a tu mejor versión. 💪`,
  (name, level) =>
    `🏆 *¡Nivel desbloqueado para ${name}!*\n*${level}* completado con todas las letras. ¡Seguimos construyendo! 🔥💪`,
]

export async function notifyClientLevelApproved(payload: {
  client_id:   string
  client_name: string
  level_title: string
}): Promise<SlackResult> {
  ...
  if (!channel?.slack_channel_id) {
    const error = "No se encontró el canal de Slack de este cliente — ¿está sincronizado? (POST /api/admin/omni/slack/sync)"
    await logJobRun(sb, "slack:notifyClientLevelApproved", "error", error)
    return { ok: false, error }
  }

  const firstName = payload.client_name.trim().split(/\s+/)[0] || payload.client_name
  const template = LEVEL_UP_TEMPLATES[Math.floor(Math.random() * LEVEL_UP_TEMPLATES.length)]
  const message = template(firstName, payload.level_title)

  const result = await postToChannel(channel.slack_channel_id, [section(message)], message)
  await logJobRun(sb, "slack:notifyClientLevelApproved", result.ok ? "ok" : "error", result.error)
  return result
}
