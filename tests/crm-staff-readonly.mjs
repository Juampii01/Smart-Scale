...
async function cleanup(accounts, clientId, prospectIds) {
  for (const id of prospectIds) await admin.from("client_crm_prospects").delete().eq("id", id)
...
async function main() {
  ...
  try {
    const { data: prospect, error: insErr } = await admin
      ...
      .insert({ client_id: clientId, name: "Prospecto de prueba", stage: "conversacion" })
      .select("id").single()
    ...
    const { data: ownRead } = await CLIENT.client.from("client_crm_prospects").select("id").eq("id", prospect.id)
    check("cliente lee su propio prospecto", (ownRead ?? []).length === 1)
    const { data: ownUpdate } = await CLIENT.client.from("client_crm_prospects").update({ notes: "actualizado por el dueño" }).eq("id", prospect.id).select("id")
    check("cliente edita su propio prospecto", (ownUpdate ?? []).length === 1)

    console.log("\n2. Staff interno puede LEER (solo lectura):")
    const { data: staffRead, error: staffReadErr } = await STAFF.client.from("client_crm_prospects").select("id").eq("id", prospect.id)
    check("staff lee el prospecto sin error", !staffReadErr && (staffRead ?? []).length === 1)

    console.log("\n3. Staff interno NO puede escribir (ni admin):")
    const { data: staffUpdate } = await STAFF.client.from("client_crm_prospects").update({ notes: "hackeado por staff" }).eq("id", prospect.id).select("id")
    check("la base niega el update de staff (0 filas afectadas)", (staffUpdate ?? []).length === 0)
    const { data: afterStaffUpdate } = await admin.from("client_crm_prospects").select("notes").eq("id", prospect.id).single()
    ...
    const { data: staffInsert, error: staffInsertErr } = await STAFF.client
      .from("client_crm_prospects").insert({ client_id: clientId, name: "prospecto falso de staff" }).select("id")
    check("la base niega el insert de staff", !!staffInsertErr || (staffInsert ?? []).length === 0)

    const { data: staffDelete } = await STAFF.client.from("client_crm_prospects").delete().eq("id", prospect.id).select("id")
    check("la base niega el delete de staff (0 filas afectadas)", (staffDelete ?? []).length === 0)
    const { data: stillThere } = await admin.from("client_crm_prospects").select("id").eq("id", prospect.id).maybeSingle()
