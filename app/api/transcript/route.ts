...
type ApifyResult = { transcript: string | null; failure: { provider: string | null; reason?: string; debug?: string } | null }
...
async function callApifyOnce(videoId: string, token: string): Promise<ApifyResult> {
  try {
    ...
    if (!res.ok) {
      return { transcript: null, failure: { provider: "apify", reason: `apify_failed_${res.status}`, debug: rawText } }
    }
    ...
    try {
      ...
      return {
        transcript: null,
        failure: { provider: "apify", reason: "apify_parse_failed", debug: `${err?.message ?? String(err)} | raw=${rawText}` },
      }
    }
    ...
    if (transcript) return { transcript, failure: null }
    ...
    return { transcript: null, failure: { provider: "apify", reason: "apify_exception", debug: err?.message ?? String(err) } }
  }
}
...
async function getYouTubeTranscript(
  ...
  const preferredFailure = (() => {
    if (watchPageResult.reason === "login_required") {
      ...
      // required" sea el muro anti-bot de YouTube contra la IP del servidor que
      // una restricción real del video — no se lo afirmamos al usuario como tal.
